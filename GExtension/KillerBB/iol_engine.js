// ==========================================
// File: iol_engine.js
// Version: 2.1 (Background Unthrottled Worker)
// ==========================================

if (!window._iol_engine_injected) {
  window._iol_engine_injected = true;
  ...
  // --- UNTHROTTLED WORKER SETUP ---
  // We create a web worker from a string to act as our background metronome
  if (!window._iol_worker) {
    const workerCode = `
      let timer = null;
    ...
    window._iol_worker = new Worker(URL.createObjectURL(blob));
  }

  // Define the start function 
  window.startIOLEngine = function() {
    if (window._iol_engine_running) {
      console.log("⚡ IOL Engine already running. Resuming operations...");
      window._iol_paused = false;
      window._iol_kill = false;
      return;
    }
    ...
    // Attach logic to the worker's tick event
    window._iol_worker.onmessage = () => {
      if (window._iol_kill) {
        console.log("🛑 Engine Killed via UI.");
        window._iol_worker.postMessage('stop'); // Stop the metronome
        window._iol_engine_running = false;
        return;
      }

      if (window._iol_paused) return; // Skip logic if paused
        ...
        win.addEventListener("click", (e) => {
          if (e.target && typeof e.target.closest === 'function') {
            ...
            if (isExitClick && window._iol_engine_running) {
              ...
              window._iol_worker.postMessage('stop');
              window._iol_engine_running = false; 
    ...
    // Tell the worker to start ticking
    window._iol_worker.postMessage('start');
    console.log("🚀 Modular IOL Engine (Unthrottled Background Worker) Injected and Running.");
  };
}

// Always attempt to start the engine when the extension injects the file
window.startIOLEngine();